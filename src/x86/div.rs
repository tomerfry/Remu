...
fn $sub(a: $t, b: $t) -> ($t, bool, bool, bool) {
    ...
    (r, b > a, (a ^ b) & (a ^ r) & $sign != 0, (a ^ b ^ r) & 0x10 != 0)
}
...
impl Cpu {
    ...
    fn $cord(&mut self, mut tmpa: $t, tmpb: $t, mut tmpc: $t) -> Result<($t, $t, bool), ()> {
    ...
    fn $negate(&mut self, mut tmpa: $t, mut tmpb: $t, mut tmpc: $t, mut neg: bool, skip: bool) -> ($t, $t, $t, bool) {
        if !skip {
    ...
    fn $postdiv(&mut self, tmpa: $t, tmpb: $t, tmpc: $t, carry: bool, neg: bool) -> Result<($t, $t), ()> {
        // 1c4: quotient out of signed range → fault.
        if !carry {
            return Err(());
        }
        // 1c5..1c8: the remainder takes the sign of the dividend.
        let tmpa = if tmpb & $sign != 0 { $neg(tmpa).0 } else { tmpa };
...
div_width!(u8, 0x80, set_szp8, sub8_fl, neg8_v, rcl8_v, cord8, negate8, prediv8, postdiv8);
div_width!(u16, 0x8000, set_szp16, sub16_fl, neg16_v, rcl16_v, cord16, negate16, prediv16, postdiv16);
...
impl Cpu {
    ...
    pub(crate) fn div8(&mut self, divisor: u8, signed: bool) -> bool {
        ...
        let Ok((c, a, carry)) = self.cord8(tmpa, tmpb, tmpc) else { return false };
    ...
    pub(crate) fn div16(&mut self, divisor: u16, signed: bool) -> bool {
        ...
        let Ok((c, a, carry)) = self.cord16(tmpa, tmpb, tmpc) else { return false };
        let (mut rem, tmpc) = (a, c);
        let mut quot = !tmpc;
        if signed {
