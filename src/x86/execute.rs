...
impl Cpu {
    ...
    pub(crate) fn dispatch<B: Bus>(&mut self, bus: &mut B, opcode: u8) -> u32 {
        match opcode {
            ...
            0x80 | 0x82 => {
                ...
                if op.is_mem() {
                    (if wb { 17 } else { 10 }) + self.ea_cycles
                } else {
                    4
                }
            }
            0x81 | 0x83 => {
                ...
                let b = if opcode == 0x81 {
                    ...
                    self.fetch_byte(bus) as i8 as u16
                };
            ...
            0x40..=0x47 => {
                ...
                let r = self.inc16(v);
                self.regs.set_reg16(opcode & 7, r);
                2
            }
            0x48..=0x4F => {
                let v = self.regs.reg16(opcode & 7);
            ...
            0x88 => {
                ...
                if op.is_mem() { 9 + self.ea_cycles } else { 2 }
            }
            0x89 => {
                let (m, op) = self.modrm(bus);
            ...
            0x8D => {
                // LEA (register form is undefined; leave the register alone)
                let (m, op) = self.modrm(bus);
                if let Operand::Mem { off, .. } = op {
            ...
            0xA1 => {
                let off = self.fetch_word(bus);
                let seg = self.seg_or(reg::DS);
                self.regs.ax = self.read16(bus, seg, off);
                10
            }
            0xA2 => {
                let off = self.fetch_word(bus);
            ...
            0xC6 => {
                ...
                let v = self.fetch_byte(bus);
                self.write_op8(bus, op, v);
                if op.is_mem() { 10 + self.ea_cycles } else { 4 }
            }
            0xC7 => {
                let (_, op) = self.modrm(bus);
                let v = self.fetch_word(bus);
            ...
            0xD2 => {
                let c = self.regs.cx as u8 as u32;
                self.shift_grp8(bus, Some(c))
            }
            0xD3 => {
                let c = self.regs.cx as u8 as u32;
                self.shift_grp16(bus, Some(c))
            }
            ...
            0xCE => {
                if self.regs.flags.contains(Flags::OF) {
            ...
            0xD6 => {
                ...
                let v = if self.regs.flags.contains(Flags::CF) {
                    0xFF
                } else {
                    0
                };
            ...
            0xD8..=0xDF => {
                // ESC: 8087 operand fetch; no coprocessor attached
                let (_, op) = self.modrm(bus);
                if let Operand::Mem { seg, off } = op {
                    let _ = self.read8(bus, seg, off);
                }
                if op.is_mem() { 8 + self.ea_cycles } else { 2 }
            }
            ...
            0xE0 => {
                ...
                if self.regs.cx != 0 && !self.regs.flags.contains(Flags::ZF) {
                    self.regs.ip = self.regs.ip.wrapping_add(rel);
                    19
                } else {
                    5
            ...
            0xE4 => {
                let p = self.fetch_byte(bus) as u16;
                let v = bus.io_read(p);
                self.regs.set_reg8(0, v);
                10
            }
            0xE5 => {
                let p = self.fetch_byte(bus) as u16;
                self.regs.ax = self.io_read16(bus, p);
                10
            }
            0xE6 => {
                let p = self.fetch_byte(bus) as u16;
            ...
            0xEC => {
                let v = bus.io_read(self.regs.dx);
                self.regs.set_reg8(0, v);
                8
            }
            0xED => {
            ...
            0xEE => {
                bus.io_write(self.regs.dx, self.regs.reg8(0));
                8
            }
            0xEF => {
                let (p, v) = (self.regs.dx, self.regs.ax);
                self.io_write16(bus, p, v);
                8
            }
            ...
            0xF5 => {
                self.regs.flags.toggle(Flags::CF);
                2
            }
            0xF8 => {
            ...
            0xFA => {
                self.regs.flags.remove(Flags::IF);
                2
            }
            0xFB => {
                self.regs.flags.insert(Flags::IF);
                self.inhibit_interrupts = true;
                2
            }
            0xFC => {
            ...
            0xF6 => {
                ...
                match m.reg() {
                    ...
                    4 => {
                        self.mul8(v);
                        70 + if mem { 6 + ea } else { 0 }
                    }
                    5 => {
                        self.imul8(v);
                        90 + if mem { 6 + ea } else { 0 }
                    }
                    6 => {
                        if !self.div8(v, false) {
                            self.interrupt(bus, 0);
                        }
                        85 + if mem { 6 + ea } else { 0 }
                    }
                    _ => {
                        if !self.div8(v, true) {
                            self.interrupt(bus, 0);
                        }
                        110 + if mem { 6 + ea } else { 0 }
                    }
                }
            }
            0xF7 => {
                ...
                match m.reg() {
                    ...
                    4 => {
                        self.mul16(v);
                        118 + if mem { 6 + ea } else { 0 }
                    }
                    5 => {
                        self.imul16(v);
                        144 + if mem { 6 + ea } else { 0 }
                    }
                    6 => {
                        if !self.div16(v, false) {
                            self.interrupt(bus, 0);
                        }
                        155 + if mem { 6 + ea } else { 0 }
                    }
                    _ => {
                        if !self.div16(v, true) {
                            self.interrupt(bus, 0);
                        }
                        176 + if mem { 6 + ea } else { 0 }
                    }
                }
            }
            ...
            0xFF => {
                ...
                match m.reg() {
                    ...
                    3 => {
                        // CALL far indirect (register form is undefined; keeps CS)
                    ...
                    4 => {
                        self.regs.ip = self.read_op16(bus, op);
                        if mem { 18 + ea } else { 11 }
                    }
                    5 => {
                        // JMP far indirect
                        let (off, seg) = self.far_operand(bus, op);
                    ...
                    _ => {
                        // PUSH r/m16 (/7 is an undocumented alias of /6)
                        let v = match op {
    ...
    fn op_rm_r8<B: Bus>(&mut self, bus: &mut B, f: fn(&mut Cpu, u8, u8) -> u8, wb: bool) -> u32 {
        ...
        if op.is_mem() {
            (if wb { 16 } else { 9 }) + self.ea_cycles
        } else {
            3
        }
    }
    ...
    fn far_operand<B: Bus>(&mut self, bus: &mut B, op: Operand) -> (u16, u16) {
        match op {
            Operand::Mem { seg, off } => (
                self.read16(bus, seg, off),
                self.read16(bus, seg, off.wrapping_add(2)),
            ),
    ...
    fn rep_str<B: Bus>(
        ...
        while self.regs.cx != 0 {
            ...
            self.regs.cx = self.regs.cx.wrapping_sub(1);
            cycles += per;
            if cmp && self.regs.flags.contains(Flags::ZF) != cont_on_zf {
                break;
            }
        }
        cycles
